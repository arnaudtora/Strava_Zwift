...
with open(file_id, "r") as fichier:
	...
	for line in contenu:
		...
		if( (line.startswith("__DEJA_AJOUTEE_"))):
			continue
		elif( (line.startswith("https://wandrer.earth/activities")) or  (line.startswith("https://www.strava.com/activities")) ):
			id = line.split("/")[-1:][0]
		elif( line.isdigit() ):
			id = line
		else:
...
print("\n\n")
for id in list_id_act[0:5]:
	print ("ID a ajouter : '" + str(id) + "'")



	# Fin d'upload OK, on met a jour le fichier
	contenu_file_out = []
	with open(file_id, "r") as fichier:
		contenu = fichier.readlines()
		for line in contenu:
